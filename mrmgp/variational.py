...
class SLDSStructuredMeanFieldVariationalPosterior(VariationalPosterior):
    ...
    def __init__(self, model, datas, xs,
                 inputs=None, masks=None, tags=None,
                 initial_variance=0.01):
        ...
        self.D = model.D
        self.K = model.K
        self.x_across = model.x_across
        self.x_within = model.x_within
        ...
        self._inferred_discrete_state_params = None
        ...
        self._inferred_continuous_state_params = None
    ...
    @property
    def inferred_discrete_state_params(self):
        return self._inferred_discrete_state_params
    ...
    @inferred_discrete_state_params.setter
    def inferred_discrete_state_params(self, value):
        # assert isinstance(value, list) and len(value) == len(self.datas)
        for prms in value:
            for key in ["pi0", "Ps", "log_likes"]:
                assert key in prms
        self._inferred_discrete_state_params = value

        # Rerun the HMM smoother with the updated parameters
        self._inferred_discrete_expectations = \
            [hmm_expected_states(prms["pi0"], prms["Ps"], prms["log_likes"])
             for prms in self._inferred_discrete_state_params]
    ...
    @property
    def inferred_continuous_state_params(self):
        return self._inferred_continuous_state_params
    ...
    @inferred_continuous_state_params.setter
    def inferred_continuous_state_params(self, value):
        # assert isinstance(value, list) and len(value) == len(self.datas)
        for prms in value:
            for key in ["J_ini", "J_dyn_11", "J_dyn_21", "J_dyn_22", "J_obs",
                        "h_ini", "h_dyn_1", "h_dyn_2", "h_obs"]:
                assert key in prms
        self._inferred_continuous_state_params = value

        # Rerun the Kalman smoother with the updated parameters
        self._inferred_continuous_expectations = \
            [kalman_info_smoother(prms["J_ini"], prms["h_ini"], 0,
                                  prms["J_dyn_11"], prms["J_dyn_21"], prms["J_dyn_22"],
                                  prms["h_dyn_1"], prms["h_dyn_2"], 0,
                                  prms["J_obs"], prms["h_obs"], 0)
             for prms in self._inferred_continuous_state_params]
    ...
    def _initialize_continuous_state_params_random(self, data, input, mask, tag):
        ...
        try:
            ...
            for i in range(xhat.shape[1]):
                ...
                elif self.x_across <= i < self.x_across+self.x_within[0]:
                    index = self.x_across * \
                        self.num_dims[0] + (i-self.x_across)*self.num_dims[1]
                elif self.x_across + self.x_within[0] <= i < self.num_groups * self.x_across + self.x_within[0]:
                    index = self.x_across*self.num_dims[0]+self.x_within[0]*self.num_dims[1] + (
                        i-self.x_across-self.x_within[0])*self.num_dims[0]
                else:
    ...
    def _initialize_continuous_state_params(self, data, x, input, mask, tag):
        T = data.shape[0]
        D = self.D
    ...
    @property
    def mean_inferred_discrete_states(self):
        full_expectations = self._inferred_discrete_expectations
        return [exp[0] for exp in full_expectations]

    @property
    def mean_inferred_continuous_states(self):
        full_expectations = self._inferred_continuous_expectations
        return [exp[1] for exp in full_expectations]
    ...
    def sample_inferred_continuous_states(self):
        return [kalman_info_sample(prms["J_ini"], prms["h_ini"], 0,
                                   prms["J_dyn_11"], prms["J_dyn_21"], prms["J_dyn_22"],
                                   prms["h_dyn_1"], prms["h_dyn_2"], 0,
                                   prms["J_obs"], prms["h_obs"], 0)
